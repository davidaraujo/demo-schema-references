...
public class WithdrawalService implements Runnable {
    ...
    public WithdrawalService(String resourcesDir,
    ...
    public static void main(final String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        if (numberThreads == 0) {
            withdrawalService = new WithdrawalService(resourcesDir, confluentPropsFile, topicName, clientId);
            new Thread(withdrawalService).start();
        }
        // run multiple producer threads
        else for (int i = 0; i < numberThreads; i++) {
            withdrawalService = new WithdrawalService(resourcesDir, confluentPropsFile, topicName, clientId + "." + i);
            new Thread(withdrawalService).start();
        }
        try {
